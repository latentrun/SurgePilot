...
- `docs/sdd/slices/P2-06-lan-first-deployment-usability.md` through `docs/sdd/adr/ADR-0018-p2-lan-first-release-bootstrap.md`, amended for the new-release Runtime default by `docs/sdd/adr/ADR-0020-p2-release-dual-runtime-default.md`, for release `up` configuration confirmation by `docs/sdd/adr/ADR-0021-p2-release-up-configuration-confirmation.md`, and for separate user-local installation by `docs/sdd/adr/ADR-0024-p2-user-local-release-installer.md`.
...
P2-06 is limited to the tagged-release LAN-first startup contract: an interactive first-run host/port confirmation with a warned canonical-loopback evaluation exception, persisted node-facing URL/port consistency, a Demo-off release default, explicit required node-facing origins, default authenticated InfluxDB publication, a strict session-cookie transport value, non-interactive complete-`.env` fixtures, a new-release `amd64,arm64` Runtime default without an architecture prompt, every valid release `up` printing its bounded non-secret configuration with read-only default-Yes confirmation and explicit interactive four-field standard-LAN updates, and the separate ADR-0024 installation entry before the unchanged `up` contract. It does not authorize network discovery, a Web/API configuration surface, a package-manager or system installer, automatic or non-interactive rewriting, advanced-HTTPS quick editing, non-network changes, or public workflow dispatch capability.
