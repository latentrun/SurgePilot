...
@pytest.mark.anyio
async def test_scenario_api_rejects_inline_script_text(client: AsyncClient) -> None:
    csrf, workspace_id, _user_id = await register(client, "scenario-script-text@example.com")
    ...
    assert "scriptText" in str(body["details"])
