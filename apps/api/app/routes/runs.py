...
@router.get(
    ...
    response_model=RunReportDetail,
    response_model_by_alias=True,
    responses={
        400: ERROR_RESPONSE,
        401: ERROR_RESPONSE,
...
def get_run_report_route(
    ...
    attach_workspace_header(response, workspace.id)
    report = get_run_report(db, workspace_id=workspace.id, run_id=run_id)
    db.commit()
    return report
...
@router.get(
    "/{runId}/monitoring",
    operation_id="getRunMonitoringLink",
    response_model=RunMonitoringLinkResponse,
    response_model_by_alias=True,
    responses={
        400: ERROR_RESPONSE,
        401: ERROR_RESPONSE,
...
def get_run_monitoring_link_route(
    run_id: RunIdPath,
    response: Response,
    db: DbDep,
    workspace: CurrentWorkspaceDep,
    ...
    validate_run_id(run_id)
    attach_workspace_header(response, workspace.id)
    result = get_run_monitoring_link(db, workspace_id=workspace.id, run_id=run_id)
    db.commit()
    return result
...
def list_run_artifacts(
    ...
    db: DbDep,
    workspace: CurrentWorkspaceDep,
    artifact_type: Annotated[RunArtifactType | None, Query(alias="artifactType")] = None,
    cursor: Annotated[str | None, Query()] = None,
    limit: LimitQuery = 50,
    sort: Annotated[Literal["createdAt", "-createdAt"], Query()] = "createdAt",
) -> RunArtifactListResponse:
    validate_run_id(run_id)
    attach_workspace_header(response, workspace.id)
    result = list_run_artifacts_report(
        db,
        workspace_id=workspace.id,
        run_id=run_id,
...
@router.patch(
    "/{runId}/validity",
    operation_id="patchRunValidity",
    response_model=RunValidityPatchResponse,
    response_model_by_alias=True,
    responses={
        400: ERROR_RESPONSE,
        401: ERROR_RESPONSE,
...
def patch_run_validity(
    run_id: RunIdPath,
    payload: RunValidityPatchRequest,
    request: Request,
    response: Response,
    db: DbDep,
    user: CurrentUserDep,
