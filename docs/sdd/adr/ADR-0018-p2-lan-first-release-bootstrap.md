...
The reconstructed P2-06 verification for this decision is recorded in
`docs/sdd/slices/P2-06-lan-first-deployment-usability.md` §21. It covers interactive and
non-interactive first run, LAN host normalization including bare and single-bracket IPv6, canonical
loopback local-only evaluation, the seven persisted quickstart values with process-level overrides
rejected, deployment-state reuse, stable Compose project identity, strict session-cookie parsing,
and non-destructive failure ordering. Release configuration, wrapper, bootstrap, node-facing,
Compose, and workflow checks live in `tests/test_release_preflight.py`,
`tests/test_release_wrapper.py`, `tests/test_bootstrap_deployment_env.py`,
`tests/test_node_facing_startup.py`, `tests/contract/test_p2_05_distribution.py`, and
`apps/api/tests/test_p0_00_services.py`; the release-stack LAN smoke lives in
`scripts/verify_p2_05_release_stack.py`. Native Linux amd64/arm64 LAN acceptance and the installer
smoke remain environment-dependent and outside default `make verify`.
