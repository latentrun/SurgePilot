...
## P1-06 Verification Backfill

The P1-06 reconstruction remains within the governance boundary established here and does not add archive extraction, presigned URLs, HTTP Range, background preprocessing, object-key or MinIO endpoint exposure, a `dependency_file.previewed` audit event, or any P2 surface. Its verification atomic is represented by focused API service checks for common text, no-extension and common source filenames, the known-binary extension denylist without a storage body read, NUL/binary markers, invalid UTF-8, ASCII and multibyte-boundary truncation without lossy replacement, and stream close/release; route checks for a CSRF-free `GET`, invalid ID, not found, cross-Workspace `404 RESOURCE_NOT_FOUND`, `503 STORAGE_UNAVAILABLE`, and the absence of bucket/object-key leakage; generated-contract freshness checks for `previewDependencyFile` and the preview enums; Web component checks for lazy loading, plain-text rendering of HTML, the truncated notice, unsupported and storage-error states with Download retained, copy behavior, and stale-preview suppression; and an E2E smoke for text preview and the binary unavailable state. The preview route stays a bounded read-only read of at most `DEPENDENCY_FILE_PREVIEW_MAX_BYTES` through the existing server-side MinIO client and adds no migration. This backfill authorizes no P2 surface and does not change the P0 execution loop, Run state machine, or artifact ingest.
