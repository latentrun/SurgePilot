...
R15 reconstruction verification backfill:

1. Session token management coverage is recorded in `apps/api/tests/test_p2_02_api_tokens_api.py` for self-only list/create/revoke, CSRF-protected writes, one-time PAT plaintext, and the absence of `secretHash` or plaintext leakage in list responses.
2. Public PAT bearer authentication, scope denial, actor-status invalidation, explicit/single-allowlisted Workspace resolution, structured writes through existing services, Run create/stop/report through the existing state machine and idempotency paths, the bounded Dependency File list/upload/delete surface, and the credential-free public Load Node summary are recorded in `apps/api/tests/test_p2_02_public_api.py`.
3. Node-facing origin canonicalization, connectivity-summary source/readiness tracking, and remote Run start fail-closed behavior are recorded in `apps/api/tests/test_p2_02_load_node_connectivity.py`.
4. Public artifact path/token/secret exclusion and byte-identical skill-snapshot alignment are recorded in `tests/contract/test_p2_02_public_api_openapi.py`; internal session-token and connectivity operation coverage is recorded in `tests/contract/test_openapi_contract.py`. The auditable operation map remains `tests/contract/public_api_operation_coverage.json`.
5. Web coverage is recorded in `apps/web/src/features/account/api-keys.test.tsx` and `apps/web/src/features/load-nodes/load-node-connectivity-summary.test.tsx`.
6. Verification commands: `make test` (contract tests), `make lint`, `make contracts-stale-check`, and `make verify`.
7. Deployment `.env`/bootstrap/Makefile startup governance, the node-facing startup Make wiring, and the SSH lifecycle verifier (`scripts/verify_p2_02_public_api_lifecycle.py` and `tests/test_p2_02_public_api_lifecycle_verifier.py`) belong to the later startup checkpoint and are intentionally not implemented by R15. At R15 the internal artifact also does not yet export the `loadNodeApiBaseUrl` System Settings field into its component schemas; the session-only `LoadNodeConnectivitySummary` operation is present.
