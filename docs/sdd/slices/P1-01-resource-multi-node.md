...
11. Runner terminal cleanup now creates a node-local diagnostic `artifacts/artifacts.zip` before terminal callback, using only the fixed allowlist `logs/runner.log`, Taurus/JMeter logs, final stats CSV variants, Debug HTTP trace/body blobs, and `*.jtl` files under the Run directory. Hidden files, temp files, symlinks, path traversal, secrets, pidfiles, runner tokens, execution control inputs, and existing archive files are excluded. The archive is skipped as `skipped_oversize` when compressed size exceeds `SURGEPILOT_RUNNER_ARCHIVE_MAX_BYTES` (default `50 MiB`, clamped by the existing hard artifact limit), without blocking other artifacts or terminal convergence.
...
16. Supplemental cross-slice verifier added after P2-01: `scripts/verify_p2_01_openapi_two_node_e2e.py` reuses the P1-01 two-node SSH stack and stop/assertion helpers to validate that a Scenario populated from materialized OpenAPI Step drafts can run through the existing two-node Standard Run path. This adds no new Resource behavior and remains an opt-in E2E command via `make verify-p2-01-openapi-two-node-e2e`.
